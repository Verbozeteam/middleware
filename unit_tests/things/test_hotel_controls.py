...
class TestHotelControls(BaseTestFramework):
    ...
    def test_controls(self):
        ...
        # turn buttons on
        self.system.fake_controllers[0].send_json({"thing": self.hotel_controls.id, "do_not_disturb": 1, "room_service": 1})
        ...
        # turn buttons off
        self.system.fake_controllers[0].send_json({"thing": self.hotel_controls.id, "do_not_disturb": 0})
        self.wait_for_condition(lambda self:
        ...
        self.system.fake_controllers[0].send_json({"thing": self.hotel_controls.id, "room_service": 0})
        self.wait_for_condition(lambda self:
            self.hotel_controls.get_state()["do_not_disturb"] == 0 and
    ...
    def test_card(self):
        # test DND and room service
        self.hotel_controls.input_ports[self.hotel_controls.hotel_card] = 10 # rapid reading
        self.hotel_controls.nocard_power_timeout = 0 # immediately time out
        self.system.arduino_emulator.sync_board()
